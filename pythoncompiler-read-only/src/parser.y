...
%token<String> FLOAT ID INT STRING 
...
%type<Node> for_stmt id id_list id_list0 if_stmt if_tail 
%type<Node> lhs or_test
%type<Node> pair primary print_target simple_stmt slicing subscription suite
%type<Node> target target_list target_opt term tuple try_stmt while_stmt 

%type<List> comparisons except_clauses exprs exprs0 ids 
...
expr :
          "lambda" id_list0 ':' expr  %prec "lambda"
                                { $$ = makeLambda (@1, $2, $4); }
...
strings :
          STRING
        | strings STRING        { $1->insert ($1->size ()-1, *$2, 1, 
                                              $2->size ()-2); 
                                  delete $2;
                                  $$ = $1; }
        ;
...
for_stmt :
          "for" lhs "in" expr_or_tuple ':' 
             loop_context suite else_part
                       { $$ = makeFor (@1, $2, $4, $7, $8); popContext (); }
        ;        
...
def_stmt :
		  ...
		  pushContext (-BREAK, -CONTINUE, +RETURN, -CLASS, -IMPORT, 0); }
            block	{ $$ = makeDef (@1, $2, $4, $8); popContext (); }
